...
from utils.util import real_batch
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        self.model.train()
        all_loss = 0
        for step, batch in tqdm(enumerate(self.train_dataloader), "training", len(self.train_dataloader), ncols=100, unit="step"):
        ...
        torch.save(self.model, './out/saved/models/KRED/checkpoint%04d.pt'%epoch)
        print("Epoch %03d, all_loss: %.3f"% (epoch, all_loss.detach().cpu().numpy()))
    ...
    def _valid_epoch(self, epoch):
        ...
        for start in tqdm(start_list, "evaluating", len(start_list), ncols=100):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs+1):
            print("Epoch: %03d/%03d"%(epoch, self.epochs))
            self._train_epoch(epoch)
            if epoch % vaild_interval == 0:
