...
from utils.util import my_collate_fn, prepare_device, real_batch
...
def multi_task_training(config, data):
    ...
    while (pretrain_epoch < 5):
        ...
        optimizer = optim.Adam(model.parameters(), lr=config['optimizer']['lr'], weight_decay=config['optimizer']['l2_regular'])
        total_loss_u2i = 0
        model.train()
